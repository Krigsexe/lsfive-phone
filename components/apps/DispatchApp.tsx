            ...
            <div className="p-2 space-y-2 overflow-y-auto flex-grow">
                {alerts.length > 0 ? (
                    alerts.map((alert) => {
                        ...
                        return (
                            <div key={alert.id} className="bg-neutral-900/80 rounded-xl p-3 border-l-4" style={{ borderColor: deptInfo.color.replace('bg-', '').split('-')[0] === 'blue' ? '#3b82f6' : deptInfo.color.replace('bg-', '').split('-')[0] === 'red' ? '#ef4444' : deptInfo.color.replace('bg-', '').split('-')[0] === 'orange' ? '#f97316' : '#6b7280' }}>
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${deptInfo.color}`}>
                                            <DeptIcon size={20} />
                                        </div>
                                        <div>
                                    ...
                                    <span className="text-xs text-slate-400 flex-shrink-0 ml-2">{alert.timestamp}</span>
                                </div>
                                <div className="mt-2 flex items-center gap-2 text-slate-400 text-sm pl-12">
                                    <MapPin size={14} />

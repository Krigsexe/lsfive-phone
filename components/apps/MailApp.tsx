            ...
            case 'read':
                ...
                return (
                     <div className="h-full flex flex-col bg-transparent">
                        <header className="p-2 bg-neutral-900/80 backdrop-blur-xl flex items-center gap-2 sticky top-0 border-b border-neutral-800 z-10">
                            <button onClick={() => setView('inbox')} className="text-white p-2 rounded-full hover:bg-neutral-700">
                                <ChevronLeft size={24} />
                            </button>
                            <h2 className="flex-grow font-semibold text-base text-white truncate">{selectedMail.subject}</h2>
                            <button onClick={() => handleDeleteMail(selectedMail.id)} className="text-white p-2 rounded-full hover:bg-neutral-700">
                                <Trash2 size={20} />
                            </button>
                        </header>
                        <div className="p-3 overflow-y-auto text-white space-y-3 bg-black/20 flex-grow text-sm">
            ...
            default:
                return (
                    <div className="h-full flex flex-col bg-transparent text-white">
                        ...
                        <ul className="flex-grow overflow-y-auto px-1 space-y-0.5">
                            {mailList.length > 0 ? mailList.map(mail => (
                                <li key={mail.id} onClick={() => handleSelectMail(mail)} className="flex items-center gap-2 p-2.5 rounded-lg hover:bg-white/10 cursor-pointer">
                                    {!mail.isRead && <div className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0"></div>}
                                    <div className={`flex-grow ${mail.isRead ? 'ml-3' : ''}`}>
                                        <div className="flex justify-between items-baseline">
                                            <p className={`font-semibold truncate text-sm ${mail.isRead ? 'text-neutral-300' : 'text-white'}`}>{mail.from}</p>
                                            <p className="text-xs text-neutral-500 flex-shrink-0">{mail.timestamp}</p>
                                        </div>
                                        <p className={`font-medium truncate text-sm ${mail.isRead ? 'text-neutral-400' : 'text-white'}`}>{mail.subject}</p>
                                        <p className="text-xs text-neutral-500 truncate">{mail.body}</p>
                                    </div>
                                </li>
                            )) : (
...
const ComposeView: React.FC<{onSend: (to: string, subject: string, body: string) => void, onBack: () => void}> = ({onSend, onBack}) => {
    ...
    return (
        <div className="h-full flex flex-col bg-black text-white">
            <header className="p-2 flex justify-between items-center sticky top-0 bg-neutral-900 border-b border-neutral-800">
                <button onClick={onBack} className="text-blue-400 px-2 py-1 text-base">{t('cancel')}</button>
